...
and coordinate discovery (exact bounding boxes and center click targets) for coding agents.
"""
...
# --- Win32 Window Enumeration Helpers ---
...
class UIAutomationInspector:
    ...
    def get_window_rect(self, hwnd: int) -> Tuple[int, int, int, int]:
        """Returns [x, y, width, height] for a window handle."""
        rect = RECT()
    ...
    def is_window_responsive(self, hwnd: int) -> bool:
        """Returns True if the window is processing messages and not hung."""
        return not bool(self._user32.IsHungAppWindow(hwnd))

    def get_window_title(self, hwnd: int) -> str:
        """Retrieves window text/title."""
    ...
    def get_window_pid(self, hwnd: int) -> int:
        """Retrieves the Process ID owning the window."""
        pid = wintypes.DWORD()
    ...
    def find_window(self, pattern: str) -> Optional[Dict[str, Any]]:
        """
        Finds the first visible window matching the regex/substring pattern.
        """
    ...
    def focus_window(self, hwnd: int) -> bool:
        """Brings the window to foreground and sets active focus."""
    ...
    def inspect_child_elements(self, parent_hwnd: int) -> List[Dict[str, Any]]:
        """
        Enumerates child controls and elements for a parent window.
        """
    ...
    def find_element(self, query: str, parent_hwnd: Optional[int] = None) -> Optional[Dict[str, Any]]:
        """
        Finds a UI control by title/text pattern.
        """
...
if __name__ == "__main__":
    ...
    for w in wins[:5]:
        print(f" - [{w['hwnd']}] '{w['title']}' at {w['rect']} (PID: {w['pid']})")
